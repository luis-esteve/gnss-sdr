...
TEST(GNSS_Block_Factory_Test, InstantiateGpsL1CaDllFllPllMulticorrelatorTracking)
{
    std::shared_ptr<InMemoryConfiguration> configuration = std::make_shared<InMemoryConfiguration>();
    configuration->set_property("Tracking.implementation", "GPS_L1_CA_DLL_FLL_PLL_Multicorrelator_Tracking");
    gr::msg_queue::sptr queue = gr::msg_queue::make(0);
    std::unique_ptr<GNSSBlockFactory> factory;
    std::shared_ptr<GNSSBlockInterface> trk_ = factory->GetBlock(configuration, "Tracking", "GPS_L1_CA_DLL_FLL_PLL_Multicorrelator_Tracking", 1, 1, queue);
    std::shared_ptr<TrackingInterface> tracking = std::dynamic_pointer_cast<TrackingInterface>(trk_);
    EXPECT_STREQ("Tracking", tracking->role().c_str());
